...
def main(review_file_path, business_file_path, output_a_path, output_b_path):
    ...
    averageStarsRDD = reviewRDD.join(businessRDD).map(lambda x: (x[1][1], (x[1][0], 1))).repartition(NUMBER_OF_PARTITIONS).reduceByKey(lambda a, b: (a[0]+b[0], a[1]+b[1])).map(lambda x: (x[0], x[1][0] / x[1][1])).persist()

    start_time = time.time()
    city_rating = averageStarsRDD.collect()
    city_rating = sorted(city_rating, reverse=False, key=lambda x: x[0])
    city_rating = sorted(city_rating, reverse=True, key=lambda x: x[1])
    m1_time = time.time()-start_time

    start_time = time.time()
    sorted_city_rating = averageStarsRDD.sortByKey(ascending=True).sortBy(lambda x: x[1], ascending=False).take(10)
