...
def main(input_file_path, output_file_path):
    ...
    output = {}
    jsonRDD = sc.textFile(input_file_path).map(json.loads).map(lambda x: {'business_id': x['business_id']}).repartition(NUMBER_OF_PARTITIONS)
    ...
    jsonRDD.map(lambda x: (x['business_id'], 1)).reduceByKey(lambda a, b: a+b).sortBy(lambda x: x[1], ascending=False).take(10)
    ...
    start_time = time.time()
    jsonRDD.map(lambda x: (x['business_id'], 1)).reduceByKey(lambda a, b: a+b).partitionBy(NUMBER_OF_PARTITIONS, lambda k: hash(k)).sortBy(lambda x: x[1], ascending=False).take(10)
